...
def trainingDL(model, X, y, task = 'regression', nfolds=5, groups=None):
...
def training_nested_cv(model, X, y, parameters, task = 'regression', nfolds=5, groups=None):
    # Initialize arrays to store true labels and predictions for each fold
    all_true_labels = []
    all_predictions = []   
...
model = NeuralNetClassifier(
    ...
    module__n_filters_time=model_params['n_filters_time'],
    module__pool_time_length=model_params['pool_time_length'],
    module__pool_time_stride=model_params['pool_time_stride'],
    module__pool_mode=model_params['pool_mode'],
    module__n_filters_spat=model_params['n_filters_spat'],
...
mean_score, all_true_labels, all_predictions, score_test = trainingDL(model, X, y, task='classification', nfolds=3, groups=groups)
