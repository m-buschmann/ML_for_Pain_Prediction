...
report.add_raw(raw=raw, title="Raw at start", psd=True) 
...
#filter data
#raw.filter(l_freq=1, h_freq=100) #später?
...
"""epochs_good.drop_bad(reject=dict(eeg=500e-6)) #doesnt work bc just drops all epochs
fig = epochs_good.plot_drop_log()
report.add_figure(fig, "Dropped epochs")"""
...
raw_cont.set_eeg_reference(ref_channels='average')
...
#apply RANSAC to the filtered epochs, identify and mark bad channels
...
#raw_cont.info['bads'] = raw_info['bads']
raw_cont.info['bads'] = bad_channels[:]
#raw.info['bads'] = bad_channels #wirklich hier? 
...
ic_indices = [i for i, (proba, label) in enumerate(zip(ic_labels["y_pred_proba"], ic_labels["labels"])) if proba > 0.8 and label != "brain"]
...
# Get the updated ICA.exclude list after applying to raw_cont
#ica.exclude = [ica.ch_names.index(ch) for ch in ica.exclude]


#check if any components were excluded
...
#plot in report
report.add_raw(raw=raw_cont, title="Raw after ICA", psd=True)"""


#_______________________________________________________________________________________________________
#apply to original data

#filter the cleaned continuous data ?
#raw.filter(l_freq=1, h_freq=100)

#also drop bad epochs?

#interpolate bad channels in the raw data
#raw.interpolate_bads(reset_bads=True) #maybe here?
...
raw = mne.io.RawArray(np.hstack(epochs.get_data()), raw.info)
...
#plot in report
report.add_raw(raw=raw, title="Raw after ICA", psd=True)
...
report.add_epochs(windows, title="Cleaned Epochs") #why still powerline noise?
