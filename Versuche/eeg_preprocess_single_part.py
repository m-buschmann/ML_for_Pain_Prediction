...
for task in ["thermalrate", "auditoryrate", "thermal", "auditory", "rest"]:
    ...
    #stats_frame.loc[p, "n_bad_chans"] = len(raw.info["bads"])
    ...
    """new_row = pd.DataFrame(
        {
            ...
            "n_bads_ica": [n_bads_ica],
        }
    )

    stats_frame = pd.concat([stats_frame, new_row], ignore_index=True)
    """
    """new_data = {
            "Participant": p,
            "Subtask": task,
...
#stats_frame = pd.DataFrame(data_list)   
# Save statistics
stats_frame.to_csv(opj(derivpath, "preprocess_stats_"+ p +".csv"))
